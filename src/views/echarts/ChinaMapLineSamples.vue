    ...
    export default {
                   ...
                   /* geo: { // geo配置方式, 简单
                    ...
                    series: [{
                        ...
                        data: [{  // 指定来源和目标位置
                            from: '福州',
                            toName: '北京',
                            coords: [[119.4543, 25.9222], [116.4551, 40.2539]]
                        }, {
                            from: '福州',
                            toName: '西安',
                            coords: [[119.4543, 25.9222], [109.1162, 34.2004]]
                        }]
                    }, {
                        // 标点信息
                        name: '点',
        ...
        mounted() {
            ...
            this.chinaMapChart.on("click", p => {
                if (this.isProvince(p.name)) {
                    this.showProvince(p.name, this.getProvinceKey(p.name));
                } else if (this.isCity(p.name)) {
                    this.showCitys(p.name, this.getCityKey(p.name));
                } else {
                    alert('找不到' + p.name)
                }
        ...
        methods: {
            /*是否省级*/
            isProvince(name) {
                return name in this.provinceMap;
            },
            isCity(name) {
                return name in this.cityMap;
            },
            getProvinceKey(name) {
                return this.provinceMap[name];
            },
            getCityKey(name) {
                return this.cityMap[name];
            },
            async showProvince(pName, keyName) {
                const provinceMapJson = await import('@/static/echarts/map/json/province/' + keyName + '.json');
                echarts.registerMap(pName, provinceMapJson);

                this.defaultMapOptions.geo.map = pName;
                this.chinaMapChart.setOption(this.defaultMapOptions);
            },
            async showCitys(pName, keyName) {
                // 显示县级地图
                const cityMapJson = await import('@/static/echarts/map/json/city/' + keyName + '.json');
                echarts.registerMap(pName, cityMapJson);
                this.defaultMapOptions.geo.map = pName;
                this.chinaMapChart.setOption(this.defaultMapOptions);
            }
        }
